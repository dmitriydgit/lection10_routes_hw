...
export class FilmService {
	...
	searchData(srchStr: string, dataCategory: string, page: number) {
		...
		return dataCategory === "films" ? this.searchPopularFilms(srchStr, dataCategory, page) : this.searchPopularActors(srchStr, dataCategory, page)
	}

	//https://api.themoviedb.org/3/movie/299536?api_key=0994e7679a856150aadcecf7de489bce&language=en-US
	getItemById(id, dataCategory) {
		return dataCategory === "films" ? this.http.get(`${this.movieUrl}/${id}${this.paramsFilmsSearch}`) : this.http.get(`${this.personUrl}/${id}${this.paramsFilmsSearch}`)
	}
